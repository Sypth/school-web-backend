...
class UndergraduateController extends Controller
{
    ...
    public function store(Request $request)
    {
        try{
            ...
            return response()->json([
                'message' => 'Undergraduate section created successfully',
            ], 201);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to create undergraduate section',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    {
        try{
            $undergraduateSection = AcademicsPage::latest()->where('section_name', $sectionName)->firstOrFail();

            return response()->json([
                'undergraduateSectionData' => $undergraduateSection,
            ], 200);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to retrieve undergraduate section data',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
