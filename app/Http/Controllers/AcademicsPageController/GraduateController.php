...
class GraduateController extends Controller
{
    ...
    public function store(Request $request)
    {
        try{
            ...
            return response()->json([
                'message' => 'Graduate section created successfully',
            ], 201);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to create graduate section',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    {
        try{
            $graduateSection = AcademicsPage::latest()->where('section_name', $sectionName)->firstOrFail();

            return response()->json([
                'graduateSectionData' => $graduateSection,
            ], 200);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to fetch graduate section',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
