...
class ElementaryController extends Controller
{
    ...
    public function store(Request $request)
    {
        try{
            ...
            return response()->json([
                'message' => 'Elementary section created successfully',
            ], 201);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to create elementary section',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    {
        try{
            $elementarySection = AcademicsPage::latest()->where('section_name', $sectionName)->firstOrFail();

            return response()->json([
                'elementarySectionData' => $elementarySection,
            ], 200);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to fetch elementary section',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
