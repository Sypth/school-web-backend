...
class JuniorHighController extends Controller
{
    ...
    public function store(Request $request)
    {
        try{
            ...
            return response()->json([
                'message' => 'Junior high section created successfully',
            ], 201);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to create junior high section',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    {
        try{
            $juniorHighSection = AcademicsPage::latest()->where('section_name', $sectionName)->firstOrFail();

            return response()->json([
                'juniorHighSectionData' => $juniorHighSection,
            ], 200);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to retrieve junior high section',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
