...
class CcisResearchController extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        try {
            ...
            return response()->json([
                'message' => 'Ccis research section created successfully.',
            ], 201);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to fetch ccis research section',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    {
        try {
            $ccisResearchSection = ResearchPage::latest()->where('section_name', $sectionName)->firstOrFail();

            return response()->json([
                'ccisResearchSectionData' => $ccisResearchSection,
            ], 200);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to fetch ccis research section',
