...
class CassedResearchController extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        try {
            ...
            return response()->json([
                'message' => 'Cassed research section created successfully.',
            ], 201);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to fetch cassed research section',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    {
        try {
            $cassedResearchSection = ResearchPage::latest()->where('section_name', $sectionName)->firstOrFail();

            return response()->json([
                'cassedResearchSectionData' => $cassedResearchSection,
            ], 200);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to fetch cassed research section',
