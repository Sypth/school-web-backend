...
class CobResearchController extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        try{
            ...
            return response()->json([
                'message' => 'Cob research section created successfully.',
            ], 201);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to fetch cob research section',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    {
        try{
            $cobResearchSection = ResearchPage::latest()->where('section_name', $sectionName)->firstOrFail();

            return response()->json([
                'cobResearchSection' => $cobResearchSection,
            ], 200);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to fetch cob research section',
