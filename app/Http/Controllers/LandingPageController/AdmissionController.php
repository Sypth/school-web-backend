...
class AdmissionController extends Controller
{
    ...
    public function store(Request $request)
    {
        try{
            ...
            return response()->json([
                'message' => 'Admission section created successfully',
            ], 201);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to create admission section',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    {
        try{
            $admissionSection = LandingPage::latest()->where('section_name', $sectionName)->firstOrFail();

            return response()->json([
                'admissionSectionData' => $admissionSection->contents
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Failed to fetch admission section',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
